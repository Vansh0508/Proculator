...
import { getLocationFromPincode } from '../services/geminiService';
...
const LocationInput: React.FC<LocationInputProps> = ({ label, value, onChange, hasWarning }) => {
  const [debouncedPincode, setDebouncedPincode] = useState(value.pincode);
  
  // Use a ref to track the current value to avoid stale closures in the async callback
  ...
  const handleLookup = useCallback(async (pincode: string) => {
    if (pincode.length < 3) return; 

    // Access latest state via ref
    const currentState = valueRef.current;
    onChange({ ...currentState, loading: true });
    
    const location = await getLocationFromPincode(pincode);
    
    // Check ref again before updating to ensure we don't overwrite newer user input
    // or if the component unmounted/changed significantly.
    const latestState = valueRef.current;
    ...
    if (location) {
      ...
      onChange({ ...latestState, loading: false });
    }
  }, [onChange]); 

  // Trigger lookup when debounced pincode changes
  useEffect(() => {
    // Only auto-fetch if we have a pincode and it looks like a new one (or city is missing)
